...
- `system_prompt.sample.md` → `system_prompt.md` にリネームまたはコピー
...
ただし `system_prompt.md` は Codex ホームにはコピーされません。ラッパーが各リクエストごとに読み込み、組み込みの既定ルールに続けて system prompt として自動注入します。
...
補足
- Compose は既定で `CODEX_APPROVAL_POLICY=never` と `CODEX_SANDBOX_MODE=danger-full-access` を設定し、コンテナ内の Codex を無人実行できるようにします。高信頼前提の構成なので、隔離されたコンテナ環境でのみ利用してください。
...
- CODEX_APPROVAL_POLICY: Codex CLI に渡す承認ポリシーの上書き設定（任意）。主な値は `untrusted` / `on-request` / `never` です。
  - 未設定の場合は Codex CLI の既定値を使います。
  - 隔離済みのコンテナや CI で承認プロンプトを完全に無効化したい場合は `never` を設定します。
...
- CODEX_SYSTEM_PROMPT_FILE: ラッパーの組み込み system prompt に追記するテキストファイル（任意）。未設定時は `CODEX_WRAPPER_PROFILE_DIR` 配下、または既定の `workspace/codex_profile/` 配下にある `system_prompt.md` を探します。
- CODEX_SYSTEM_PROMPT: 組み込みルールとファイルベース system prompt の後ろに追記するインライン文字列（任意）。
